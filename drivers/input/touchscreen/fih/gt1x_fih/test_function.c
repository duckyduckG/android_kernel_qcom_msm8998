...
static unsigned char _check_full_screen_jitter(void)
{
	...
	/* if 6sigama>jitter_limit or max_jitter>jitter_limit+10jitter is not legal */
		...
		if (test_error_code & _GT_SHORT) {
			...
			for (index = 0; index < shortresult[0]; index++) {
				...
				if (shortresult[2 + index * 4] & 0x80) {
					...
					bytes += (s32) sprintf((char *)&data[bytes], "Drv%d - ", shortresult[2 + index * 4] & 0x7F);
					...
					if (shortresult[2 + index * 4] == (sys.max_driver_num + 1)) {
						...
						bytes += (s32) sprintf((char *)&data[bytes], "GND\\VDD%d - ");

					}

					else {

						bytes += (s32) sprintf((char *)&data[bytes], "Sen%d - ", shortresult[2 + index * 4] & 0x7F);
